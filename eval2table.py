...
def attn(path):
    return func_names[
        next(re.finditer(r"(so|sp|ts)-(so|sp|ts)", path)).group(0).split("-")[0]
    ]
...
def out(path):
    return func_names[
        next(re.finditer(r"(so|sp|ts)-(so|sp|ts)", path)).group(0).split("-")[1]
    ]
...
def main():
    ...
    results = [results_table(path, opt.task, opt.columns) for path in opt.results_paths]

    all_results = [setting_results(setting, opt.task == "morph") for setting in results]
